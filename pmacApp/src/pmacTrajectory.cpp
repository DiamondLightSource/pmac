...
asynStatus pmacTrajectory::initialise(int noOfPoints)
{
  ...
  if (status == asynSuccess){
    ...
    if (profileUser_ == NULL){
      debug(DEBUG_ERROR, functionName, "Unable to allocate memory for the trajectory scan user mode array");
      status = asynError;
    }
  }
  ...
  if (status == asynSuccess){
    ...
    if (profileVelMode_ == NULL){
      debug(DEBUG_ERROR, functionName, "Unable to allocate memory for the trajectory scan velocity mode array");
      status = asynError;
    }
  }
...
asynStatus pmacTrajectory::append(double **positions, double *times, int *user, int *velocity, int noOfPoints)
{
  asynStatus status = asynSuccess;
  int axis = 0;
  int counter = 0;
  ...
  if (status == asynSuccess){
    ...
    while (counter < noOfPoints && status == asynSuccess){
      ...
      if (velocity[counter] > 0xF){
        debug(DEBUG_ERROR, functionName, "Invalid velocity mode value (> 4 bit)", velocity[counter]);
        status = asynError;
      }
      counter++;
    }
